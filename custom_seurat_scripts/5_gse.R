...
getBM(attributes = c("ensembl_gene_id",
                     "external_gene_name",
                     "entrezgene_id"), mart = mart) -> mouse_ensembl_2_geneName
names(mouse_ensembl_2_geneName) <- c("ensembl_id", "gene_name", "entrez_id")
...
GO_BP_GSEA[[i]] <- fgsea::fgseaMultilevel(pathways = GO_BP$genesets,
                                          stats = rank_list[[i]],
                                          minSize = 1,
                                          maxSize = Inf,
                                          eps = 0,
...
GO_CC_GSEA <- list()
for (i in 1:length(rank_list)){
GO_CC_GSEA[[i]] <- fgsea::fgseaMultilevel(pathways = GO_CC$genesets,
                                          stats = rank_list[[i]],
                                          minSize = 1,
                                          maxSize = Inf,
                                          eps = 0,
...
GO_MF_GSEA <- list()
for (i in 1:length(rank_list)){
GO_MF_GSEA[[i]] <- fgsea::fgseaMultilevel(pathways = GO_MF$genesets,
                                          stats = rank_list[[i]],
                                          minSize = 1,
                                          maxSize = Inf,
                                          eps = 0,
...
KEGG_GSEA <- list()
for (i in 1:length(rank_list)){
KEGG_GSEA[[i]] <- fgsea::fgseaMultilevel(pathways = KEGG$genesets,
                                    stats = rank_list[[i]],
                                    minSize = 1,
                                    maxSize = Inf,
                                    eps = 0,
...
# write.table(file = paste(path, names(data_list)[i], "_GO_BP_genes.csv"),
#             sep = ";", quote = FALSE, row.names = FALSE)
# GO_BP_GSEA[[i]] %>% as.data.frame() %>%
#   dplyr::mutate(leadingEdge = paste0(leadingEdge)) %>%
#   dplyr::select(-leadingEdge) %>%
# write.table(file = paste(path, names(data_list)[i], "_GO_BP.csv"),
#             sep = ";", quote = FALSE, row.names = FALSE)
# 
# GO_CC_GSEA[[i]] %>% as.data.frame() %>%
#                 dplyr::mutate(leadingEdge = paste0(leadingEdge)) %>%
# write.table(file = paste(path, names(data_list)[i], "_GO_CC_genes.csv"),
#             sep = ";", quote = FALSE, row.names = FALSE)
# GO_CC_GSEA[[i]] %>% as.data.frame() %>%
#   dplyr::mutate(leadingEdge = paste0(leadingEdge)) %>%
#   dplyr::select(-leadingEdge) %>%
# write.table(file = paste(path, names(data_list)[i], "_GO_CC.csv"),
#             sep = ";", quote = FALSE, row.names = FALSE)
# 
# GO_MF_GSEA[[i]] %>% as.data.frame() %>%
#                 dplyr::mutate(leadingEdge = paste0(leadingEdge)) %>%
# write.table(file = paste(path, names(data_list)[i], "_GO_MF_genes.csv"),
#             sep = ";", quote = FALSE, row.names = FALSE)
# GO_MF_GSEA[[i]] %>% as.data.frame() %>%
#   dplyr::mutate(leadingEdge = paste0(leadingEdge)) %>%
#   dplyr::select(-leadingEdge) %>%
# write.table(file = paste(path, names(data_list)[i], "_GO_MF.csv"),
#             sep = ";", quote = FALSE, row.names = FALSE)
...
KEGG_GSEA[[i]] %>% as.data.frame() %>%
  dplyr::mutate(leadingEdge = paste0(leadingEdge)) %>%
  dplyr::select(-leadingEdge) %>%
write.table(file = paste(path, names(data_list)[i], "_KEGG.csv"),
            sep = ";", quote = FALSE, row.names = FALSE)
}
...
for (i in 1:length(rank_list)){
PPInfer::GSEA.barplot(GO_BP_GSEA[[i]],
  category = "pathway",
  score = "NES",
  top = 30,
  pvalue = "padj",
  sort = "padj",
  numChar = 100) +
  theme(plot.margin = margin(10, 10, 10, 50)) -> GO_BP_plot_list[[i]]
}

GO_CC_plot_list <- list()
for (i in 1:length(rank_list)){
PPInfer::GSEA.barplot(GO_CC_GSEA[[i]],
  category = "pathway",
  score = "NES",
  top = 30,
  pvalue = "padj",
  sort = "padj",
  numChar = 100) +
  theme(plot.margin = margin(10, 10, 10, 50)) -> GO_CC_plot_list[[i]]
}

## bar charts
GO_MF_plot_list <- list()
for (i in 1:length(rank_list)){
PPInfer::GSEA.barplot(GO_MF_GSEA[[i]],
  category = "pathway",
  score = "NES",
  top = 30,
  pvalue = "padj",
  sort = "padj",
  numChar = 100) +
  theme(plot.margin = margin(10, 10, 10, 50)) -> GO_MF_plot_list[[i]]
}

## bar charts
KEGG_plot_list <- list()
for (i in 1:length(rank_list)){
PPInfer::GSEA.barplot(KEGG_GSEA[[i]],
  category = "pathway",
  score = "NES",
  top = 30,
  pvalue = "padj",
